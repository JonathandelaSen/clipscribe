...
import { listConnectedYouTubeRelatedVideos } from "@/lib/youtube/server";
...
export async function GET(request: NextRequest) {
  try {
    const result = await listConnectedYouTubeRelatedVideos(request.cookies.get(YOUTUBE_SESSION_COOKIE)?.value);
    ...
    const response = NextResponse.json(
      {
        ok: false,
        error: error instanceof Error ? error.message : "Failed to load YouTube videos.",
      },
      { status: 401 }
    );
    response.cookies.delete(YOUTUBE_SESSION_COOKIE);
